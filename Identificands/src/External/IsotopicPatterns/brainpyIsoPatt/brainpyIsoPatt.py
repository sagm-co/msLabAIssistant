...
from brainpyIsoPatt.composition import (
    PyComposition,
    parse_formula,
    calculate_mass,
...
def neutral_mass(mz, z, charge_carrier):
    return (mz * abs(z)) - (z * charge_carrier)


def mass_charge_ratio(neutral_mass, z, charge_carrier):
...
class Element(object):
    def __init__(self, symbol,isoLibrary, isotopes=None):
        self.symbol = symbol
        self.isotopes = isotopes or _isotopes_of(symbol,isoLibrary)
...
def make_fixed_isotope_element(element, neutrons,isoLibrary):
    
    isotope = element.isotopes[neutrons - element.isotopes[0].neutrons]
    el = Element(element.symbol + ("[%d]" % neutrons),isoLibrary, OrderedDict([
...
def _isotopes_of(element,isoLibrary):
    freqs = dict()
    mono_neutrons = None
    element_data = isoLibrary[element]
...
class IsotopicConstants(dict):
    def __init__(self, order,isoLibrary):
        self._order = 0
        self.order = order
        self.isoLibrary=isoLibrary
        self.periodicTable={k: Element(k,isoLibrary) for k in isoLibrary}
        self.periodicTable["H+"] = Element("H+",isoLibrary)
    @property
    def order(self):
        return self._order
    ...
    def add_element(self, symbol):
        if symbol in self:
            return
        if symbol in self.periodicTable:
            element = self.periodicTable[symbol]
        else:
            symbol_parsed, isotope = _get_isotope(symbol)
            if isotope == 0:
                raise KeyError(symbol)
            element = make_fixed_isotope_element(self.periodicTable[symbol_parsed], isotope,self.isoLibrary)
...
def max_variants(composition,periodicTable):
    ...
    for element, count in composition.items():
        if element == "H+":
            continue
        try:
            max_n_variants += count * periodicTable[element].max_neutron_shift()
        except KeyError:
            pass
...
class IsotopicDistribution(object):
    ...
    """
    def __init__(self, composition,isoLibrary, order=-1):
        self.composition = composition
        self.isoLibrary=isoLibrary
        self._isotopic_constants = IsotopicConstants(order,self.isoLibrary)
        self.periodicTable={k: Element(k,isoLibrary) for k in isoLibrary}
        self.periodicTable["H+"] = Element("H+",isoLibrary)
        self._order = 0
        self.order = order
    ...
    @order.setter
    def order(self, value):
        max_variant_count = max_variants(self.composition,self.periodicTable)
        if value == -1:
    ...
    def _create_monoisotopic_peak(self):
        mass = calculate_mass(self.composition,self.isoLibrary)
        intensity = 0.
        for element in self.composition:
            if element == "H+":
                continue
    ...
    def aggregated_isotopic_variants(self, charge_carrier, charge=0):
        """
        ...
        isoPattern=pd.DataFrame({'mz':[],'intensity':[],'z':[]})
        ...
        for i in range(self.order + 1):
            ...
            isoPattern=pd.concat([isoPattern,
                                  pd.DataFrame({'mz' : [adjusted_mz],
                                   'intensity' : [probability_vector[i]],
                                                'z' : [charge]})
        ...
        isoPattern.intensity=isoPattern.intensity/np.max(isoPattern.intensity)
        return isoPattern


def isotopic_variants(composition, charge_carrier,isoLibrary, npeaks=5, charge=0):
    """
    ...
    """
    periodicTable={k: Element(k,isoLibrary) for k in isoLibrary}
    periodicTable["H+"] = Element("H+",isoLibrary)
    if npeaks is None:
        max_n_variants = max_variants(composition,periodicTable)
    ...
    return IsotopicDistribution(composition,isoLibrary, npeaks).aggregated_isotopic_variants(charge_carrier,charge)
